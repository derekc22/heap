...
public class MaxHeap<Generic>{
    ...
    public Generic getLargestValue(){
    ...
    public class Node implements Comparator<Node> {
        ...
        public void sink(){
            if (!this.isLeaf){
                Node largerChildNode = this.getLargerChild();

                int nodeIndex = this.index;
                int largerChildNodeIndex = largerChildNode.index;

                int diff = compare(this, largerChildNode);

                if (diff < 0){
                    swapNodes(this, largerChildNodeIndex, largerChildNode, nodeIndex);
                    this.sink();
                }
            }
        }

        public Node getLargerChild(){
